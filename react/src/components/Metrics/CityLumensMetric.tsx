...
export default function CityLumensMetric() {
  const [metricData, setMetricData] = useState<MetricResult[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [avgLumens, setAvgLumens] = useState<number | null>(null);
  ...
  const fetchLumensMetric = async () => {
    try {
      ...
      const metricResult = result?.result?.[cityId]?.["AverageLumens"];
      ...
      if (metricResult?._data && metricResult._data.length > 0) {
        ...
        if (latestValue !== null && latestValue !== undefined) {
          setAvgLumens(parseFloat(latestValue.toFixed(2)));
        }
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch lumens metric",
      );
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="c3-kendo-wrapper p-4">
        <Card>
          <CardBody>
            <div className="text-center">Loading city lumens data...</div>
          </CardBody>
        </Card>
      </div>
    );
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center w-full">
              <div></div>
              <h2 className="text-xl font-bold">City Average Lumens</h2>
              <div></div>
            </div>
          </CardHeader>
          ...
          <CardBody>
              ...
              <div className="flex justify-center items-center gap-8">
                ...
                <div className="text-center">
                  <h3 className="text-4xl font-bold text-primary">
                    {avgLumens !== null ? `${avgLumens}` : "N/A"}
                  </h3>
                  <p className="text-secondary">Average Lumens</p>
                </div>
              </div>
            </div>
            ...
            {metricData.length > 0 && (
              <Chart>
                <ChartTitle
                  text={`City Average Lumens Over Time - ${selectedMonth} 2026`}
                />
