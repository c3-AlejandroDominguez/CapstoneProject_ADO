...
export default function BuildingLumensMetric() {
  ...
  const fetchLumensMetric = async () => {
    try {
      ...
      // Evaluate the AverageLumens metric for a single building
      ...
      const payload = {
        ids: buildingId,
        expressions: ["AverageLumensBuilding"],
        interval: "DAY",
      ...
      const metricResult =
        result?.result?.[buildingId]?.["AverageLumensBuilding"];
      ...
      if (metricResult?._data && metricResult._data.length > 0) {
        ...
        // Create time series data for the chart (skip zeros, limit to last 100 points)
        const chartData = dataArray
          .map((value: number, index: number) => {
            const currentDate = new Date(tsInfo.start);
            currentDate.setDate(currentDate.getDate() + index); // Increment by DAYS not hours
            return {
  ...
  return (
    <div className="c3-kendo-wrapper p-4">
      <div
        className="shadow-xxl rounded-lg"
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center w-full">
              <div></div>
              <h2 className="text-xl font-bold">Average Lumens</h2>
              <div></div>
            </div>
          </CardHeader>
          ...
          <CardBody>
            ...
            {metricData.length > 0 && (
              <Chart>
                <ChartTitle
                  text={`Average Lumens Over Time - ${selectedMonth} 2026`}
                />
