...
export default function BuildingVoltageMetric() {
  const [metricData, setMetricData] = useState<MetricResult[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [avgVoltage, setAvgVoltage] = useState<number | null>(null);
  ...
  const fetchVoltageMetric = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const metricResult = result?.result?.[buildingId]?.["AverageVoltage"];
      ...
      if (metricResult?._data && metricResult._data.length > 0) {
        ...
        if (latestValue !== null && latestValue !== undefined) {
          setAvgVoltage(parseFloat(latestValue.toFixed(2)));
        }
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch voltage metric",
      );
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="c3-kendo-wrapper p-4">
        <Card>
          <CardBody>
            <div className="text-center">Loading voltage data...</div>
          </CardBody>
        </Card>
      </div>
    );
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center w-full">
              <div></div>
              <h2 className="text-xl font-bold">Voltage Metrics</h2>
              <div></div>
            </div>
          </CardHeader>
          ...
          <CardBody>
              ...
              <div className="flex justify-center items-center gap-8">
                ...
                <div className="text-center">
                  <h3 className="text-4xl font-bold text-primary">
                    {avgVoltage !== null ? `${avgVoltage}V` : "N/A"}
                  </h3>
                  <p className="text-secondary">Average Voltage</p>
                </div>
              </div>
            </div>
            ...
            {metricData.length > 0 && (
              <Chart>
                <ChartTitle
                  text={`Average Voltage Over Time - ${selectedMonth} 2026`}
                />
