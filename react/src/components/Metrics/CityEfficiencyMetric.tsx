...
export default function CityEfficiencyMetric() {
  const [metricData, setMetricData] = useState<MetricResult[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [avgEfficiency, setAvgEfficiency] = useState<number | null>(null);
  ...
  useEffect(() => {
    fetchEfficiencyMetric();
  }, [selectedCity, selectedMonth]);

  const fetchEfficiencyMetric = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const metricResult = result?.result?.[cityId]?.["AverageEfficiency"];
      ...
      if (metricResult?._data && metricResult._data.length > 0) {
        ...
        if (latestValue !== null && latestValue !== undefined) {
          setAvgEfficiency(parseFloat(latestValue.toFixed(2)));
        }
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to fetch efficiency metric",
      );
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="c3-kendo-wrapper p-4">
        <Card>
          <CardBody>
            <div className="text-center">Loading city efficiency data...</div>
          </CardBody>
        </Card>
      </div>
    );
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center w-full">
              <div></div>
              <h2 className="text-xl font-bold">City Average Efficiency</h2>
              <div></div>
            </div>
          </CardHeader>
          ...
          <CardBody>
              ...
              <div className="flex justify-center items-center gap-8">
                ...
                <div className="text-center">
                  <h3 className="text-4xl font-bold text-primary">
                    {avgEfficiency !== null ? `${avgEfficiency}%` : "N/A"}
                  </h3>
                  <p className="text-secondary">Average Efficiency</p>
                </div>
              </div>
            </div>
            ...
            {metricData.length > 0 && (
              <Chart>
                <ChartTitle
                  text={`City Average Efficiency Over Time - ${selectedMonth} 2026`}
                />
            ...
            {metricData.length === 0 && !loading && (
              <div className="text-center text-secondary mt-4">
                No city efficiency data available
              </div>
            )}
          </CardBody>
        </Card>
